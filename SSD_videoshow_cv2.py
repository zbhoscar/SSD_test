...
#image = caffe.io.load_image('/home/zbh/Desktop/ZBH_Midfiles/UCF101pic/Haircut/v_Haircut_g01_c04/11.jpg')
l='/home/zbh/Desktop/Datasets/UCF-101/BandMarching/v_BandMarching_g02_c05.avi'
cap=cv2.VideoCapture(l)
outfile='/home/zbh/Desktop/SSD_'+l.split("/")[-1]
fps = cap.get(cv2.CAP_PROP_FPS)
if math.isnan(fps):         # if fps = nan , set it to 25
    fps=25
wait = int(1000/fps)
size = (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)),int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)))
out = cv2.VideoWriter(outfile, cv2.VideoWriter_fourcc(*'XVID'), fps, size)
fn=1
...
while (cap.isOpened()):  
    ...
    if ret==True:
        ...
        colors=[[255,255,0],[173,255,47],[127,255,212],[0,0,205],[125,38,205],[238,203,173],[139,131,134],[255,0,0]]
        ...
        for i in xrange(top_conf.shape[0]):
            ...
            xmax = min(int(round(top_xmax[i] * image.shape[1])),size[0])
            ymax = min(int(round(top_ymax[i] * image.shape[0])),size[1])
            score = top_conf[i]
            label = int(top_label_indices[i])
            label_name = top_labels[i]
            display_txt = '%s: %.2f'%(label_name, score)

            color = colors[label%colorsnum]
            # currentAxis.add_patch(plt.Rectangle(*coords, fill=False, edgecolor=color, linewidth=2))
            # currentAxis.text(xmin, ymin, display_txt, bbox={'facecolor':color, 'alpha':0.5})
            print 'In',fn-1,'frame:',i,label_name,xmin,ymin,xmax,ymax,score#,label,display_txt, color
